  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[var(--canvas)] select-none">
      ...
      <div className="h-[50px] px-8 border-b border-[var(--border)] flex items-center gap-6 shrink-0 bg-[var(--canvas)]">
         {studioTabs.map(tab => (
           <button 
             ...
             className={`h-full flex items-center gap-2 text-[13px] font-bold transition-all relative ${helpSubTab === tab.id ? 'text-[var(--primary)]' : 'text-[var(--muted)] hover:text-[#f8fafc]'}`}
           >
             {tab.icon}
             <span>{tab.label}</span>
             {helpSubTab === tab.id && (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-[var(--canvas)]">
        {helpSubTab === 'welcome' && (
          <div className="p-10 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
             <div className="mb-12 text-center max-w-2xl mx-auto">
                <h1 className="text-4xl font-extrabold text-[#f8fafc] mb-4 tracking-tight">Focus on your code.</h1>
                <p className="text-[16px] text-[var(--muted)] font-medium leading-relaxed">ForgeMail Studio intercepts all outgoing signals from your development environment, isolation patterns guaranteed.</p>
             </div>

             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                {[
                   { title: 'Project Isolation', desc: 'Separate mail flows between multiple apps using folders.', icon: <Layers className="text-[var(--primary)]" size={24} />, color: 'bg-blue-500/10' },
                   { title: 'Smart Inspection', desc: 'Verify HTML, Source, and plain text within standard layouts.', icon: <Search className="text-purple-400" size={24} />, color: 'bg-purple-500/10' },
                   { title: 'Local Relay', desc: 'Zero external latency. Fast, secure, and always isolated.', icon: <Zap className="text-orange-400" size={24} />, color: 'bg-orange-500/10' }
                ].map((item, i) => (
                   <div key={i} className="p-8 bg-[var(--surface)] border border-[var(--border)] rounded-[2rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group">
                      ...
                      <p className="text-[13px] text-[var(--muted)] leading-relaxed font-medium">{item.desc}</p>
                   </div>
                ))}
             </div>
             ...
             <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center gap-10 shadow-sm relative overflow-hidden group hover:border-[var(--primary)]/30 transition-all">
                ...
                <div className="flex-1 text-center md:text-left">
                   <h4 className="text-[20px] font-bold text-[#f8fafc] mb-2">Explore the Setup Guide</h4>
                   <p className="text-[14px] text-[var(--muted)] font-medium leading-relaxed mb-6">Learn how to connect your Laravel, Flutter, or Node.js applications in less than 30 seconds.</p>
                   <button 
        ...
        {helpSubTab === 'integration' && (
          <div className="p-10 max-w-6xl mx-auto animate-in fade-in duration-500">
             <div className="flex items-center justify-between mb-10">
                <h1 className="text-3xl font-extrabold text-[#f8fafc] tracking-tight">Signal Configuration</h1>
                <button 
             ...
             <div className="grid grid-cols-1 lg:grid-cols-[450px_1fr] gap-10">
                <div className="space-y-6">
                   ...
                   <div className="p-6 bg-[var(--primary)]/5 border border-[var(--primary)]/10 rounded-[2rem] flex items-center gap-4">
                      ...
                      <p className="text-[13px] text-[var(--primary)] font-semibold leading-relaxed">Local-only environment. No mail ever leaves your machine unless explicitly relayed.</p>
                   </div>
                </div>
                ...
                <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[2.5rem] p-10 shadow-sm flex flex-col h-full">
                   <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8">
                      <div>
                         <h2 className="text-[11px] font-black text-[var(--muted)] uppercase tracking-wider mb-2 ml-1">Tech Ecosystem</h2>
                         <div className="relative group">
                            <button className="bg-[var(--canvas)] px-6 py-2.5 rounded-full text-[13px] font-bold text-[#f8fafc] flex items-center gap-3 hover:bg-[var(--border)] transition-all min-w-[140px] justify-between">
                      ...
                      <div>
                         <h2 className="text-[11px] font-black text-[var(--muted)] uppercase tracking-wider mb-2 ml-1">Active Workspace</h2>
                         <div className="relative group">
                            <button className="bg-[var(--canvas)] px-6 py-2.5 rounded-full text-[13px] font-bold text-[var(--primary)] flex items-center gap-3 hover:bg-[var(--border)] transition-all min-w-[160px] justify-between border border-[var(--primary)]/10">
                            ...
                            <div className="absolute top-full left-0 mt-2 w-56 bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl py-2 invisible group-hover:visible z-[100] transition-all max-h-[300px] overflow-y-auto custom-scrollbar">
                               ...
                               {projects.map(project => (
                                 <button 
                                   ...
                                   className={`w-full text-left px-5 py-3 text-[13px] transition-all font-bold flex items-center gap-2 ${selectedProject === project ? 'text-[var(--primary)] bg-[var(--primary)]/5' : 'text-[var(--muted)] hover:bg-[var(--canvas)] hover:text-[var(--primary)]'}`}
                                 >
                   ...
                   <div className="flex-1 bg-[var(--canvas)] border border-[var(--border)] rounded-[2rem] p-10 font-mono text-[14px] text-[#f8fafc] relative group shadow-inner overflow-hidden">
                      <div className="absolute top-6 right-6 z-10">
                          <button 
                            ...
                            className={`p-4 rounded-2xl transition-all hover:scale-105 active:scale-95 shadow-xl border border-[var(--border)] flex items-center gap-2 ${copiedField === 'Integration snippet captured' ? 'bg-green-500 text-white' : 'bg-[var(--surface)] text-[var(--primary)]'}`}
                          >
                   ...
                   <p className="mt-6 text-[12px] text-[var(--muted)] font-medium text-center">Refer to <span onClick={() => openExternalLink("https://github.com/khayson/SMTP")} className="text-[var(--primary)] hover:underline cursor-pointer">Studio Documentation</span> for custom ports and relay patterns.</p>
                </div>
             </div>
          </div>
        )}
        ...
        {helpSubTab === 'whatsnew' && (
          <div className="p-10 max-w-6xl mx-auto animate-in fade-in duration-500">
             <div className="bg-[var(--surface)] border border-[var(--border)] p-16 rounded-[3rem] shadow-sm relative overflow-hidden">
                <div className="relative z-10 max-w-2xl space-y-8">
                   <div className="flex items-center gap-3">
                      <div className="w-2.5 h-2.5 rounded-full bg-[var(--primary)] animate-pulse shadow-[0_0_10px_#1877F2]" />
                      <span className="text-[12px] font-bold text-[var(--primary)] uppercase tracking-[0.2em]">Flagship Version Update</span>
                   </div>
                   <h3 className="text-5xl font-extrabold text-[#f8fafc] tracking-tight leading-tight">Studio 1.2.5 <br/> Dark Forge Evolution</h3>
                   <p className="text-[16px] text-[var(--muted)] leading-relaxed font-medium">We've completed the transition to a high-fidelity dark theme, re-engineered the 3-column desktop layout, and introduced persistent 'Signals' folder state for total project isolation.</p>
                   
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                      <div className="p-6 bg-[var(--canvas)] rounded-[1.5rem] border border-[var(--border)]">
                         <Layout size={24} className="text-[var(--primary)] mb-4" />
                         <p className="text-[14px] text-[#f8fafc] font-bold">Responsive 3-Column Architecture</p>
                      </div>
                      <div className="p-6 bg-[var(--canvas)] rounded-[1.5rem] border border-[var(--border)]">
                         <Layers size={24} className="text-[var(--primary)] mb-4" />
                         <p className="text-[14px] text-[#f8fafc] font-bold">Project Isolation Folders</p>
                      </div>
                   </div>
                </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div className="p-10 bg-[var(--surface)] border border-[var(--border)] rounded-[2.5rem] shadow-sm">
                   <h4 className="text-[18px] font-bold text-[#f8fafc] mb-4">Hotline Favorites</h4>
                   <p className="text-[14px] text-[var(--muted)] font-medium leading-relaxed">Dynamic contact list that tracks your most frequent signal senders for rapid access.</p>
                </div>
                <div className="p-10 bg-[var(--surface)] border border-[var(--border)] rounded-[2.5rem] shadow-sm">
                   <h4 className="text-[18px] font-bold text-[#f8fafc] mb-4">Trash & Stars</h4>
                   <p className="text-[14px] text-[var(--muted)] font-medium leading-relaxed">Full persistence for message organization. Organize by priority or discard with focus.</p>
                </div>
             </div>
          </div>
        )}
